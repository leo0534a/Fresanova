...
export default function OrderDetailPage() {
  ...
  const confirmTransfer = async () => {
    ...
    try {
      await api.post(`/orders/${id}/confirm-transfer`);
      toast.success('Transferencia confirmada');
      loadOrder();
    } catch (error) {
      toast.error(error.response?.data?.message || 'Error al confirmar transferencia');
    } finally {
              ...
              <div key={status} className="flex items-center flex-1">
                ...
                {index < statusFlow.length - 1 && (
                  <div className={`flex-1 h-1 mx-1 rounded ${
                    index < currentIndex ? 'bg-fresanova-500' : 'bg-gray-200 dark:bg-gray-700'
                  }`} />
                )}
              </div>
            ))}
          ...
          <div className="space-y-3 text-sm">
            ...
            {order.customerInfo.neighborhood && (
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900 dark:text-white">{order.customerInfo.neighborhood}</span>
              </div>
            )}
            <div className="flex justify-between">
            ...
            {order.paymentMethod === 'transferencia' && (
              <div className="flex justify-between items-center">
                ...
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                  ...
                  {order.transferConfirmed ? 'Confirmada' : 'Pendiente'}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            {order.paymentMethod === 'transferencia' && !order.transferConfirmed && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg bg-yellow-500 hover:bg-yellow-600 text-white font-medium transition-colors disabled:opacity-50"
              >
            ...
            {order.transferProofUrl && (
              <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <p className="text-sm font-medium text-blue-700 dark:text-blue-300 mb-2">Comprobante de transferencia:</p>
                <a
        ...
        <div className="space-y-4">
          {order.items.map((item, index) => (
            <div key={item._id || index} className="flex items-start gap-4 py-3 border-b border-gray-100 dark:border-gray-700 last:border-0">
              <div className="w-8 h-8 rounded-full bg-fresanova-100 dark:bg-fresanova-900/30 text-fresanova-600 flex items-center justify-center font-bold text-sm flex-shrink-0">
                {index + 1}
              </div>
              <div className="flex-1">
                <p className="font-medium text-gray-900 dark:text-white">
                  ...
                  {item.selectedSize?.name && (
                    <span className="text-sm text-gray-500 ml-1">({item.selectedSize.name})</span>
                  )}
                </p>
                ...
                {item.toppings?.length > 0 && (
                  <p className="text-sm text-gray-500">Toppings: {item.toppings.map((t) => `${t.name} (${formatCurrency(t.price)})`).join(', ')}</p>
                )}
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600 space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            <span className="text-gray-500">
              Domicilio{order.customerInfo?.neighborhood ? ` (${order.customerInfo.neighborhood})` : ''}:
            </span>
          ...
          <div className="space-y-3">
            {order.statusHistory.map((entry, index) => (
              <div key={index} className="flex items-center gap-3 text-sm">
                <div className="w-2 h-2 rounded-full bg-fresanova-500"></div>
